...
def get_best_matching_language_voice(language_code):
    
    voice_list = load_voices()
    
    exact_match = next((voice for voice in voice_list if voice.startswith(language_code + '-')), None)
    if exact_match:
        return exact_match
    
    general_match = next((voice for voice in voice_list if voice.startswith(language_code.split('-')[0] + '-')), None)
...
def generate_mp3_file(
    ...
    if command == "tts":
        command_to_execute.append("--write-media")
        command_to_execute.append(f'"{output_mp3_file_path}"')
